...
function l(r, e) {
  const s = (r.getAttribute("style") || "").split(";").filter((o) => !!o.trim()).reduce((o, h) => {
    const f = h.split(":").map((b) => b.trim()), P = f[0], E = f[1];
    return o[P] = E, o;
  }, {}), i = Object.assign(s, e), g = Object.keys(i).filter((o) => !!o.trim()).reduce((o, h) => (o += `${h}: ${i[h]};`, o), "");
...
const c = class {
  ...
  render(e) {
    ...
    const n = this.getBarElement(t), s = this.getAppendToElement();
    if (this.translateProgressBar(n, e), !this.options.showSpinner) {
      const i = t.querySelector(a.spinnerSelector);
      i && k(i);
    }
    return s != document.body && u(s, a.customParentClassName), s && s.appendChild(t), t;
  }
  triggerRepaint() {
  ...
  set(e) {
    ...
    const n = this.render(!t), s = this.getBarElement(n), i = this.options.speed, g = this.options.easing;
    return this.triggerRepaint(), x((o) => {
      this.positionUsing === "" && (this.positionUsing = this.getPositioningCSS()), l(s, this.barPositionCSS(e, i, g)), e === 1 ? (l(n, {
        transition: "none",
        opacity: 1
  ...
  inc(e) {
    ...
    if (t) {
      if (t > 1)
        return this;
      if (typeof e != "number") {
        const s = (typeof this.options.trickleIncrementalCurve == "function" ? this.options.trickleIncrementalCurve(t) : this.options.trickleIncrementalCurve).find(
  ...
  translateProgressBar(e, t) {
    const n = t ? this.getBarPercentage(0) : this.getBarPercentage(this.getPercent() || 0);
    l(e, {
  ...
  barPositionCSS(e, t, n) {
    let s = {};
    return this.positionUsing === "translate3d" ? s = {
      transform: `translate3d(${this.getBarPercentage(e)}%,0,0)`
    } : this.positionUsing === "translate" ? s = {
      transform: `translate(${this.getBarPercentage(e)}%,0)`
    } : s = { "margin-left": `${this.getBarPercentage(e)}%` }, s.transition = `all ${t}ms ${n} 0s`, s;
  }
  getPositioningCSS() {
