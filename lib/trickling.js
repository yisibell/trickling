...
var p = (s, e, t) => (B(s, typeof e != "symbol" ? e + "" : e, t), t);
function y(s, e, t) {
...
function c(s, e) {
  const r = (s.getAttribute("style") || "").split(";").filter((n) => !!n.trim()).reduce((n, g) => {
    const f = g.split(":").map((v) => v.trim()), k = f[0], b = f[1];
    return n[k] = b, n;
  }, {}), o = Object.assign(r, e), m = Object.keys(o).filter((n) => !!n.trim()).reduce((n, g) => (n += `${g}: ${o[g]};`, n), "");
...
var a = /* @__PURE__ */ ((s) => (s.wrapperSelectorId = "trickling", s.customParentClassName = "trickling-custom-parent", s.busyFlagClassName = "trickling-busy", s.template = `
  ...
  </div>`, s.barSelector = '[role="bar"]', s.spinnerSelector = '[role="spinner"]', s.rtlClassName = "rtl", s))(a || {}), u = /* @__PURE__ */ ((s) => (s.queryBarElementError = "[Trickling]: Can not find 'barSelector' element!", s.queryAppendToElementError = "[Trickling]: Can not find 'options.appendTo' element!", s))(u || {});
const l = class {
  constructor(e) {
    p(this, "status", null);
  ...
  render(e) {
    ...
    d(document.documentElement, a.busyFlagClassName);
    const t = document.createElement("div");
    t.id = a.wrapperSelectorId, t.innerHTML = a.template, this.options.rtl && d(t, a.rtlClassName), this.options.customWrapperClassName && d(t, this.options.customWrapperClassName), this.setCSSVars(t);
    const i = this.getBarElement(t), r = this.getAppendToElement();
    if (this.translateProgressBar(i, e), !this.options.showSpinner) {
      const o = t.querySelector(a.spinnerSelector);
      o && E(o);
    }
    return r != document.body && d(r, a.customParentClassName), r && r.appendChild(t), t;
  }
  triggerRepaint() {
    const e = this.getWrapperElement();
    e && (l.progressOffsetWidth = e.offsetWidth);
  }
  set(e) {
    const t = this.isStarted();
    !t && this.visible(), e = y(e, this.options.minimum, 1), this.setPercent(e === 1 ? null : e);
    const i = this.render(!t), r = this.getBarElement(i), o = this.options.speed, m = this.options.easing;
    return this.triggerRepaint(), x((n) => {
      this.positionUsing === "" && (this.positionUsing = this.getPositioningCSS()), c(r, this.barPositionCSS(e, o, m)), e === 1 ? (c(i, {
        transition: "none",
        opacity: 1
  ...
  translateProgressBar(e, t) {
    const i = t ? this.getBarPercentage(0) : this.getBarPercentage(this.getPercent() || 0);
    c(e, {
  ...
  visible() {
    if (this.isRendered() && !this.options.removeFromDOM) {
      const e = this.getWrapperElement();
      e && c(e, { display: "block", opacity: 1 }), this.setPercent(null);
    }
  }
  hidden() {
    const e = this.getWrapperElement();
    e && (c(e, { display: "none" }), this.setPercent(null), this.translateProgressBar(this.getBarElement(e), !0));
  }
  remove(e) {
    if (P(document.documentElement, a.busyFlagClassName), !this.options.removeFromDOM && !e) {
      this.hidden();
      return;
    }
    const t = this.getAppendToElement();
    P(t, a.customParentClassName);
  ...
  getBarPercentage(e) {
    return C(e, this.options.rtl);
  }
  barPositionCSS(e, t, i) {
    let r = {};
    return this.positionUsing === "translate3d" ? r = {
      transform: `translate3d(${this.getBarPercentage(e)}%,0,0)`
    } : this.positionUsing === "translate" ? r = {
      transform: `translate(${this.getBarPercentage(e)}%,0)`
    } : r = { "margin-left": `${this.getBarPercentage(e)}%` }, r.transition = `all ${t}ms ${i} 0s`, r;
  }
  getPositioningCSS() {
...
let h = l;
p(h, "instance"), p(h, "progressOffsetWidth", 0);
const M = function(s) {
  return h.createProgress(s);
};
export {
  h as Trickling,
