...
function f(e, t) {
  const o = (e.getAttribute("style") || "").split(";").filter((r) => !!r.trim()).reduce((r, u) => {
    const m = u.split(":").map((k) => k.trim()), C = m[0], T = m[1];
    return r[C] = T, r;
...
function v(e, t) {
  return (typeof e == "string" ? e : h(e)).indexOf(" " + t + " ") >= 0;
}
function S(e, t) {
  const s = h(e), i = s + t;
...
const c = class {
  ...
  render(t) {
    ...
    const i = s.querySelector(this.barSelector), o = t ? "-100" : d(this.status || 0), n = typeof this.options.appendTo == "string" ? document.querySelector(this.options.appendTo) : this.options.appendTo;
    if (f(i, {
  ...
  set(t) {
    ...
    return c.progressOffsetWidth = i.offsetWidth, E((r) => {
      this.positionUsing === "" && (this.positionUsing = this.getPositioningCSS()), f(o, this.barPositionCSS(t, n, l)), t === 1 ? (f(i, {
        transition: "none",
        opacity: 1
      }), c.progressOffsetWidth = i.offsetWidth, setTimeout(() => {
        f(i, {
  ...
  barPositionCSS(t, s, i) {
    let o = {};
    return this.positionUsing === "translate3d" ? o = { transform: `translate3d(${d(t)}%,0,0)` } : this.positionUsing === "translate" ? o = { transform: `translate(${d(t)}%,0)` } : o = { "margin-left": `${d(t)}%` }, o.transition = `all ${s}ms ${i} 0s`, o;
  }
  getPositioningCSS() {
...
let p = c;
a(p, "instance"), a(p, "progressOffsetWidth", 0);
const $ = function(e) {
  return p.createProgress(e);
};
export {
  p as Trickling,
  $ as createTrickling
